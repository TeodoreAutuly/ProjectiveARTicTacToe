...
class Camera:
    def __init__(self, id):
        ...
        try:
            # Open the camera with the correct id using cv.VideoCapture
    ...
    def getFrame(self):
        ...
        # Get the frame using the read() method of the camera
        ret, frame = self.cap.read()

        # If there is a frame, display and return it
        if ret:
            # Display the frame using cv.imshow()
            cv.imshow("Camera", frame)
            
            # Use cv.waitKey() to display the image
            cv.waitKey(1)
            return frame
        else:
            return None
